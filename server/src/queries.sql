...
SELECT
    CASE WHEN MIN(checkpoint_time) <= (SELECT time FROM TrailMedal WHERE trail_name = LOWER(:trail_name) AND medal_type = 'rainbow') THEN TRUE ELSE FALSE END AS rainbow,
    CASE WHEN MIN(checkpoint_time) <= (SELECT time FROM TrailMedal WHERE trail_name = LOWER(:trail_name) AND medal_type = 'gold') THEN TRUE ELSE FALSE END AS gold,
    CASE WHEN MIN(checkpoint_time) <= (SELECT time FROM TrailMedal WHERE trail_name = LOWER(:trail_name) AND medal_type = 'silver') THEN TRUE ELSE FALSE END AS silver,
    CASE WHEN MIN(checkpoint_time) <= (SELECT time FROM TrailMedal WHERE trail_name = LOWER(:trail_name) AND medal_type = 'bronze') THEN TRUE ELSE FALSE END AS bronze
FROM
    Time
    ...
    INNER JOIN (
        ...
        FROM
            SplitTime
            INNER JOIN Time ON Time.time_id = SplitTime.time_id
        WHERE
            LOWER(Time.trail_name) = LOWER(:trail_name)
...
WHERE
    LOWER(Time.trail_name) = LOWER(:trail_name)
    AND checkpoint_num = max_checkpoint
